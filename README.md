...
系统按年授权：**一个卡密对应一个家庭**（家庭内多台设备用同一 PIN 登录，不限设备数）。

1. 部署完成后打开网页 → 切到「创建家庭」→ 输入自家 PIN + 卖家提供的激活卡密（格式 `MATH-XXXX-XXXX-XXXX`）。
2. 激活成功后即可使用，有效期一年；到期前系统会提示剩余天数，到期后需联系卖家续费。
3. 同一卡密只能激活一次（绑定到创建时的机器与 PIN）；家人加入家庭只需 PIN，无需卡密。
4. 卡密校验需要访问卖家的卡密服务器（`docker-compose.yml` 的 `LICENSE_SERVER_URL`）；已激活家庭在断网时仍可正常使用到到期日。
