...
- **三种练习模式**
  - 练习模式：每答对一题 +1，答错不扣分。
  - 计时模式：每题限时，≤3 秒答对 +2，>3 秒答对 +1，答错不扣分。
  - 闯关模式：正确率 ≥80% 通关 +10；全对额外 +5（共 +15）。
- **积分系统**：答对才加分、答错不扣；每天最高累计 100 分（次日重置）；错题订正不再加分，订正对后移出错题本。
- **错题本**：自动收集错题，可一键订正。
...
系统按年授权：**一个卡密对应一个家庭**（家庭内多台设备用同一 PIN 登录，不限设备数）。卡密为**离线签名卡密**（HMAC 签名 + 内置有效期），本机验签激活，**无需联网、无需任何服务器**。

1. 卖家用 `node tools/genkey.cjs [天数]` 生成卡密（默认 365 天）发给买家。**卡密可提前批量生成，不影响有效期。**
2. 买家部署完成后打开网页 → 切到「创建家庭」→ 输入自家 PIN + 激活卡密（格式 `MATH-XXXX-XXXX-XXXX`）。
3. 激活成功后即可使用；**有效期从买家激活（创建家庭）那一刻起算**，默认一年。到期前系统会提示剩余天数，到期后需联系卖家续费（新卡密在续费页输入即可）。
4. 同一卡密在本机只能激活一个家庭；家人加入家庭只需 PIN，无需卡密。

> 说明：离线卡密无法远程吊销、也防不住卡密被复制到别的机器激活（熟人场景够用）。若需严格防共享，可考虑中心服务器方案（自行扩展）。
