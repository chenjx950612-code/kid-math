...
story.append(section_table([
    ['模式', '说明'],
    ['练习', '普通练习，不计时。答对 +1，答错不扣分。'],
    ['计时', '限时练习，答对按速度计分：≤3 秒 +2，>3 秒 +1，答错不扣分。'],
    ['闯关', '达到 80% 正确率即通关 +10 分；全部答对（100%）额外 +5 分（共 +15）。'],
], colw=[30*mm, 135*mm]))
...
story.append(section_table([
    ...
    ['计时（>3 秒答对）', '+1'],
    ['闯关通关（正确率≥80%）', '额外 +10'],
    ['闯关全对（正确率=100%）', '再额外 +5（共 +15）'],
    ['错题订正', '不再加分'],
    ['兑换礼品', '扣除对应积分'],
], colw=[55*mm, 110*mm]))
story.append(callout('⚠️ 每日积分上限：每个孩子每天获得的积分（实际净增）累计最多 100 分。达到上限后不再增加，第二天本地时间零点自动重置为 0。该上限是为了防止孩子一天刷太多积分。', bg=colors.HexColor('#fff4e6'), border=CORAL))
